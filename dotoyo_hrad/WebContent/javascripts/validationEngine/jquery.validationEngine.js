...
(function ($) {
    ...
    $.fn.validationEngine = function (settings) {
        ...
        $(this).bind("submit", function (caller) { // ON FORM SUBMIT, CONTROL AJAX FUNCTION IF SPECIFIED ON DOCUMENT READY
        	...
        	//判断表单是否重复提交
        	var formFlagSelector = "#"+this.id+"Flag";
        	
            $.validationEngine.onSubmitValid = true;
            $.validationEngine.settings = settings;
            if ($.validationEngine.submitValidation(this, settings) == false) {
            	
            	var duplicatedFormFlag = $(formFlagSelector);
            	if (duplicatedFormFlag != null){
                	var duplicatedFormFlagValue = $(formFlagSelector).val();
                	if (duplicatedFormFlagValue == "0"){
                		$(formFlagSelector).val("1");
                	}
                	else if(duplicatedFormFlagValue == "1"){
                		alert("请勿重复提交表单.");
                		return false;
                	}	
            	}
    ...
    $.validationEngine = {
        defaultSetting: function (caller) { // NOT GENERALLY USED, NEEDED FOR THE API, DO NOT TOUCH
            ...
            function _required(caller, rules) { // VALIDATE BLANK FIELD
                ...
                if (callerType == "checkbox") {
                    ...
                    if ($("input[name='" + callerName + "']:checked").size() == 0) {
                        ...
                        $.validationEngine.addTop = true;//如果是checkbox 把高度增高 add by matychen
                        if ($("input[name='" + callerName + "']").size() == 1) {
                            promptText += $.validationEngine.settings.allrules[rules[i]].alertTextRadio + "<br />";
                        } else {
                            promptText += $.validationEngine.settings.allrules[rules[i]].alertTextRadio + "<br />";
                        }
                    }
            ...
            function _notconfirm(caller, rules, position) { // VALIDATE FIELD MATCH
                confirmField = rules[position + 1];
                if($(caller).attr('value') == "") return
                if ($(caller).attr('value') == $("#" + confirmField).attr('value')) {
                    $.validationEngine.isError = true;
                    promptText += $.validationEngine.settings.allrules["notconfirm"].alertText + "<br />";
                }
            }
            ...
            //验证不能今天以前日期
			function _minCurrentDay(caller, rules, position) {
				var fmt = eval(rules[position + 1]);
				...
				var currentdate = today.getFullYear() + checkToAddZero("" + Month) + checkToAddZero("" +today.getDate());
				if(value != "" && value.length > 9){
					value = value.substring(0,10);
					var das = value.substring(0,4) + "" + value.substring(5,7) + "" + value.substring(8,10);
					if(das < currentdate){
						promptText += $.validationEngine.settings.allrules["minCurrentDay"].alertText + "<br />";
						$.validationEngine.isError = true;
					}
				}else if(value != "" && value.length > 9){
					promptText += "日期格式错误<br/>";
					$.validationEngine.isError = true;
				}
			}
			function checkToAddZero(){

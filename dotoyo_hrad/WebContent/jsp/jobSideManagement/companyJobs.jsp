...
<div class="content-right">
    	<div class="administrator-title"><strong>已发布职位管理</strong></div>
    	<s:form  id="pageFrom" name="pageFrom" method="POST" action="jobSideManagementAction!viewJobs.action">
    	<s:hidden name="jobSearchVo.userId"></s:hidden>
...
function delJobs(checkboxName){
	if(checkStatus(checkboxName)==false) {
		...
		document.pageFrom.action='jobSideManagementAction!delJobByP.action';
...
function suspendJobs(checkboxName){
	if(checkStatus(checkboxName)==false) {
		...
		if(window.confirm("你确认要进行此操作吗？")){
		document.pageFrom.action='jobSideManagementAction!suspendJobsByP.action';
		document.pageFrom.method='post';
		document.pageFrom.submit();
		}
	}

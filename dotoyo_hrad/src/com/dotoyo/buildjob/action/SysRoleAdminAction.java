 ...
 * @description 管理员角色管理
 */
public class SysRoleAdminAction extends SysAction {
	 ...
	 * @return
	 */
	@AuthFunction(functionCode=FunctionCodeConstant.FUN_ADMIN_ROLE_MANAGE)
	...
	public String beforeEdit(){
		...
		tmp.setId(dto.getId());
		dto = sysRoleAdminService.search(tmp);
	...
	public String edit(){
		try{
			if(validateInput()){
				//修改数据
				sysRoleAdminService.update(dto);
				//查询所有数据
				searchData();
				clear();
				return Constants.RTN_SYS_MAIN;
	...
	public String remove(){
		...
		for(String id : removeBox){
			...
			sysRoleAdminService.remove(tmp);
		}
		searchData();
		return Constants.RTN_SYS_MAIN;
	}
	...
	private void userSearchData(){
		...
		this.userDtoList = sysUserRoleAdminService.searchList(this.userPageInfo, userSearchdto);
		//转换查询字段代码到名称
	...
	private boolean checkIsExisted(){
		SysRoleDto outDTO = sysRoleAdminService.search(dto);
		if(outDTO == null){
			return false;
		}else{

 ...
 * @description 增值服务猎证服务设置
 */
public class SysIncrementHuntCardSettingAction extends SysAction {
	private Map<String,String> serviceNames;
	private SysIncrementHuntCardSettingDto dto;
	private SysIncrementHuntCardSettingDto searchdto;
	private List<SysIncrementHuntCardSettingDto> dtoList;
	private String serviceNameMsg;
	private String timesOfLookupMsg;
	 ...
	 * @return
	 */
	@AuthFunction(functionCode=FunctionCodeConstant.FUN_INCREMENT_SERVICE_HUNTCARD)
	public String init(){
		clear();
		//查询所有数据
		searchdto = new SysIncrementHuntCardSettingDto();
		//初始化数据 - 初始化服务名称
...
//
//				if(advertSettingDto == null && hangCardSettingDto == null && recruitSettingDto == null && talentsSettingDto == null){
	...
	public String beforeAdd(){
		//清空对象
		dto = new SysIncrementHuntCardSettingDto();
	...
	public String add(){
		try{
			if(validateInput() && !checkIsExisted()){
				...
				//插入数据
				sysIncrementHuntCardSettingService.addIncrementHuntCardSetting(dto);
				//查询数据
				searchData();
				clear();
				return Constants.RTN_SYS_MAIN;
	...
	public String edit(){
		try{
			if(validateInput()){
				...
				//修改数据
				sysIncrementHuntCardSettingService.updateIncrementHuntCardSetting(dto);
				//查询所有数据
				searchData();
				clear();
				return Constants.RTN_SYS_MAIN;
	...
	public String remove(){
		if(!StringUtil.newInstance().sNull(dto.getServiceCode()).equals("")){
			...
			orderDto.setItemCode(ApplicationConstant.SERVICE_ITEM_NAME_HUNTCARD_CODE);
			...
			if(counts > 0){
				...
				//** remove records **//
			sysIncrementHuntCardSettingService.removeIncrementHuntCardSetting(dto);
				//查询数据
				searchData();
				clear();
		}
	...
	private boolean checkIsExisted(){
		SysIncrementHuntCardSettingDto outDTO = sysIncrementHuntCardSettingService.queryIncrementHuntCard(dto);
		if(outDTO == null){
			return false;
		}else{
	...
	private boolean validateInput(){
		...
		if(util.sNullTrim(dto.getTimesOfLookup()).equals("")){
			timesOfLookupMsg = "查看联系方式次数不能为空";
		}
		if(serviceNameMsg.equals("") && timesOfLookupMsg.equals("") ){
			return true;
		}
		return false;
	}
	...
	private void searchData(){
		...
		this.dtoList = sysIncrementHuntCardSettingService.queryIncrementHuntCardList(this.pageInfo, searchdto);
	}
	...
	public SysIncrementHuntCardSettingDto getDto() {
		return dto;
	}
	public void setDto(SysIncrementHuntCardSettingDto dto) {
		this.dto = dto;
	}
	public SysIncrementHuntCardSettingDto getSearchdto() {
		return searchdto;
	}
	public void setSearchdto(SysIncrementHuntCardSettingDto searchdto) {
		this.searchdto = searchdto;
	}
	public List<SysIncrementHuntCardSettingDto> getDtoList() {
		return dtoList;
	}
	public void setDtoList(List<SysIncrementHuntCardSettingDto> dtoList) {
	...
	public String getTimesOfLookupMsg() {
		return timesOfLookupMsg;
	}
	public void setTimesOfLookupMsg(String timesOfLookupMsg) {
		this.timesOfLookupMsg = timesOfLookupMsg;
	}

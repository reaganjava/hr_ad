 ...
 */
public class SysIncrementTalentsFoundOfflineSettingAction extends SysAction {
	private Map<String,String> serviceNames;
	private Map<String,String> fieldsOfSearchs;
	private SysIncrementTalentsFoundOfflineSettingDto dto;
	private SysIncrementTalentsFoundOfflineSettingDto searchdto;
	private List<SysIncrementTalentsFoundOfflineSettingDto> dtoList;
	...
	public String init(){
		clear();
		//查询所有数据
		searchdto = new SysIncrementTalentsFoundOfflineSettingDto();
		//初始化数据 - 初始化服务名称
...
//				if(advertSettingDto == null && hangCardSettingDto == null && huntCardSettingDto == null && recruitSettingDto == null){
	...
	public String beforeAdd(){
		//清空对象
		dto = new SysIncrementTalentsFoundOfflineSettingDto();
	...
	public String add(){
		try{
			if(validateInput() && !checkIsExisted()){
				...
				//插入数据
				sysIncrementTalentsFoundOfflineSettingService.addIncrementTalentsFoundSetting(dto);
				//查询数据
				searchData();
				clear();
				return Constants.RTN_SYS_MAIN;
	...
	public String edit(){
		try{
			if(validateInput()){
				...
				//修改数据
				sysIncrementTalentsFoundOfflineSettingService.updateIncrementTalentsFoundSetting(dto);
				//查询所有数据
				searchData();
				clear();
				return Constants.RTN_SYS_MAIN;
	...
	public String remove(){
		...
		if(!StringUtil.newInstance().sNull(dto.getServiceCode()).equals("")){
			...
			//** remove records **//
			sysIncrementTalentsFoundOfflineSettingService.removeIncrementTalentsFoundSetting(dto);
			//查询数据
			searchData();
			clear();
		}
	...
	private boolean checkIsExisted(){
		SysIncrementTalentsFoundOfflineSettingDto outDTO = sysIncrementTalentsFoundOfflineSettingService.queryIncrementTalentsFound(dto);
		if(outDTO == null){
			return false;
		}else{
	...
	private void searchData(){
		...
		this.dtoList = sysIncrementTalentsFoundOfflineSettingService.queryIncrementTalentsFoundList(this.pageInfo, searchdto);
		//转换查询字段代码到名称
		if(fieldsOfSearchs != null && fieldsOfSearchs.size() > 0 && dtoList != null){
			for(int i = 0;i < dtoList.size();i++){
				SysIncrementTalentsFoundOfflineSettingDto tmp = dtoList.get(i);
	...
	public SysIncrementTalentsFoundOfflineSettingDto getDto() {
		return dto;
	}
	public void setDto(SysIncrementTalentsFoundOfflineSettingDto dto) {
		this.dto = dto;
	}
	public SysIncrementTalentsFoundOfflineSettingDto getSearchdto() {
		return searchdto;
	}
	public void setSearchdto(SysIncrementTalentsFoundOfflineSettingDto searchdto) {
		this.searchdto = searchdto;
	}
	public List<SysIncrementTalentsFoundOfflineSettingDto> getDtoList() {
		return dtoList;
	}
	public void setDtoList(List<SysIncrementTalentsFoundOfflineSettingDto> dtoList) {

 ...
 * @description 增值服务招聘服务设置
 */
public class SysIncrementRecruitSettingAction extends SysAction {
	private Map<String,String> serviceNames;
	private SysIncrementRecruitSettingDto dto;
	private SysIncrementRecruitSettingDto searchdto;
	private List<SysIncrementRecruitSettingDto> dtoList;
	private String serviceNameMsg;
	private String timesOfPublicPositionMsg;
	private String timesOfLookupBlogMsg;
	private String timesOfLookupInviteMsg;
	 ...
	 * @return
	 */
	@AuthFunction(functionCode=FunctionCodeConstant.FUN_INCREMENT_SERVICE_RECRUIT)
	public String init(){
		clear();
		//查询所有数据
		searchdto = new SysIncrementRecruitSettingDto();
		//初始化数据 - 初始化服务名称
...
//
//				if(advertSettingDto == null && hangCardSettingDto == null && huntCardSettingDto == null && talentsSettingDto == null){
				if(ApplicationConstant.USER_TYPE_ENTERPRISE.equalsIgnoreCase(tmp.getServiceType())){
	...
	public String beforeAdd(){
		//清空对象
		dto = new SysIncrementRecruitSettingDto();
	...
	public String add(){
		try{
			if(validateInput() && !checkIsExisted()){
				...
				//插入数据
				sysIncrementRecruitSettingService.addIncrementRecruitSetting(dto);
				//查询数据
				searchData();
				clear();
				return Constants.RTN_SYS_MAIN;
	...
	public String edit(){
		try{
			if(validateInput()){
				...
				//修改数据
				sysIncrementRecruitSettingService.updateIncrementRecruitSetting(dto);
				//查询所有数据
				searchData();
				clear();
				return Constants.RTN_SYS_MAIN;
	...
	public String remove(){
		if(!StringUtil.newInstance().sNull(dto.getServiceCode()).equals("")){
			...
			orderDto.setItemCode(ApplicationConstant.SERVICE_ITEM_NAME_RECRUIT_CODE);
			...
			if(counts > 0){
				...
				//** remove records **//
			sysIncrementRecruitSettingService.removeIncrementRecruitSetting(dto);
				//查询数据
				searchData();
				clear();
		}
	...
	private void clear(){
		serviceNameMsg = "";
		timesOfPublicPositionMsg = "";
		timesOfLookupBlogMsg = "";
		timesOfLookupInviteMsg = "";
		priceMsg = "";
		msg = "";
	}
	/**
	...
	private boolean checkIsExisted(){
		SysIncrementRecruitSettingDto outDTO = sysIncrementRecruitSettingService.queryIncrementRecruit(dto);
		if(outDTO == null){
			return false;
		}else{
	...
	private boolean validateInput(){
		...
		if(util.sNullTrim(dto.getTimesOfPublicPosition()).equals("")){
			timesOfPublicPositionMsg = "发布职位次数不能为空";
		}
		if(util.sNullTrim(dto.getTimesOfLookupBlog()).equals("")){
			timesOfLookupBlogMsg = "查看博站次数不能为空";
		}
		if(util.sNullTrim(dto.getTimesOfLookupInvite()).equals("")){
			timesOfLookupInviteMsg = "邀请面试次数不能为空";
		}
		if(serviceNameMsg.equals("") &&
				timesOfPublicPositionMsg.equals("") &&
				timesOfLookupBlogMsg.equals("")
				&& timesOfLookupInviteMsg.equals("") ){
	...
	private void searchData(){
		...
		this.dtoList = sysIncrementRecruitSettingService.queryIncrementRecruitList(this.pageInfo, searchdto);
	}
	...
	public SysIncrementRecruitSettingDto getDto() {
		return dto;
	}
	public void setDto(SysIncrementRecruitSettingDto dto) {
		this.dto = dto;
	}
	public SysIncrementRecruitSettingDto getSearchdto() {
		return searchdto;
	}
	public void setSearchdto(SysIncrementRecruitSettingDto searchdto) {
		this.searchdto = searchdto;
	}
	public List<SysIncrementRecruitSettingDto> getDtoList() {
		return dtoList;
	}
	public void setDtoList(List<SysIncrementRecruitSettingDto> dtoList) {
	...
	public String getTimesOfPublicPositionMsg() {
		return timesOfPublicPositionMsg;
	}
	public void setTimesOfPublicPositionMsg(String timesOfPublicPositionMsg) {
		this.timesOfPublicPositionMsg = timesOfPublicPositionMsg;
	}
	public String getTimesOfLookupBlogMsg() {
		return timesOfLookupBlogMsg;
	}
	public void setTimesOfLookupBlogMsg(String timesOfLookupBlogMsg) {
		this.timesOfLookupBlogMsg = timesOfLookupBlogMsg;
	}
	public String getTimesOfLookupInviteMsg() {
		return timesOfLookupInviteMsg;
	}
	public void setTimesOfLookupInviteMsg(String timesOfLookupInviteMsg) {
		this.timesOfLookupInviteMsg = timesOfLookupInviteMsg;
	}
